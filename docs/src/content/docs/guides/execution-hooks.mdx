...
Use it for standing instructions such as a compliance note, or the memory protocol of the Pro [agent-memory feature](/products/uc-ai/docs/guides/agent-memory/). Read `uc_ai.get_exec_context` in the hook to see which agent runs. Its fields are null outside an agent run.
