...
Choose the orchestrator when the correct order of agents depends on the input, and the AI must plan this order. If the steps are always the same, a deterministic [workflow](/products/uc-ai/docs/guides/multi-agent-systems/workflows/) is simpler and cheaper.
</Aside>

UC AI gives each delegate agent to the orchestrator as a [tool](/products/uc-ai/docs/guides/tools/). This is the most flexible pattern, because the AI decides the flow from the task.

If you need the same specialist in more than one caller, or together with normal tools, register it as a permanent tool. Read the [agent as tool](/products/uc-ai/docs/guides/multi-agent-systems/agent-as-tool/) guide.
...
<Aside type="tip">
  Use a more capable model for the orchestrator, here GPT-5 Mini. It must reason
  about which agents to call. A delegate agent can use a cheaper model, because
  its task is narrow.
</Aside>
...
<Aside type="note">
  The orchestrator AI sees the `p_description` of each delegate agent when it
  decides which agent to call. Write a clear and specific description. The
  description controls how well the orchestrator delegates.
</Aside>
...
The orchestrator makes the model calls itself, and it reaches each delegate through a **tool call**. The [`uc_ai_agent_messages`](/products/uc-ai/docs/guides/multi-agent-systems/#conversation-message-log) transcript therefore records each delegate as a `tool_call` and `tool_result` pair, with the delegate agent in `tool_name`. These pairs stand around the own reasoning and the final synthesis of the orchestrator. This is a direct pattern, so `agent_code` names the orchestrator on every agent row. The `tool_name` of a tool row names the delegate.
...
- **Input schemas matter**: the `input_schema` of a delegate agent defines how the orchestrator calls it. A clear schema with a good `description` for each field helps the AI pass the correct parameters.
