create or replace package body test_uc_ai_openai_responses as
  ...
  -- Repros the Responses API failure hit by the webinar demos:
  -- a reasoning model with tools, store=false, and no encrypted reasoning.
  -- On the follow-up turn after a tool call, uc_ai_responses_api re-sends
  -- the prior response's raw output items (including reasoning items with
  -- rs_... ids) as input. OpenAI only resolves those ids when store=true,
  -- so with store=false this hits:
  --   ORA-20302 ... Item with id 'rs_...' not found. Items are not
  --   persisted when `store` is set to false.
  -- We force a reasoning item by requesting a reasoning summary, otherwise
  -- o4-mini may emit no visible reasoning item for trivial prompts.
  ...
  begin
    ...
    uc_ai.g_enable_reasoning := true;
    uc_ai_responses_api.g_reasoning_effort := 'medium';
    uc_ai_responses_api.g_reasoning_summary := 'auto';
    uc_ai_responses_api.g_store_responses := false;
    uc_ai_responses_api.g_include_encrypted_reasoning := false;
    ...
    uc_ai_test_message_utils.validate_message_array(
      p_messages => l_messages,
      p_test_name => 'Function calling with reasoning, store=false',
      p_should_have_reasoning => true
    );
    ...
    uc_ai_responses_api.g_reasoning_summary := null;
    uc_ai_responses_api.g_store_responses := true;
  end test_function_calling_reasoning;
