...
![An empty UC AI chat region: the title APEX Helper, a welcome message next to the assistant avatar, two suggested prompt chips, and a message box](../../../assets/apex-chat/chat-empty-state.png)
...
five seconds is a bad page, and a gateway can stop the request before the model
answers.
...
[Lesson 8 of the tutorial](/products/uc-ai/docs/tutorial/service-contract-desk/08-apex-page/)
...
![A finished turn in the chat region: the question of the user, a tool call with the status success, a collapsed reasoning block, the answer of the assistant, and a row with the time, the token counts, a copy button and two thumb buttons](../../../assets/apex-chat/chat-conversation.png)
...
Feedback** controls the buttons, and it is off by default.
...
| **Suggested Prompts** | Textarea | — | Example questions, one for each line. They show as buttons while the conversation is empty. A click sends the question. |
| **Auto Generate Title** | Checkbox | `N` | Names the conversation after the first answer, with the language model of the browser. |
| **Collect Feedback** | Checkbox | `N` | Shows a thumbs up and a thumbs down under the newest answer. |
...
query gives you the conversations that made people unhappy.
...
| **Welcome Message** | Textarea | `Ask me anything...` | The text above the suggested prompts, while the conversation is empty. |
