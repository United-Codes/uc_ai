...
A user says "the chat is broken". The page shows nothing you can act on. There is
no stack trace, because nothing raised an error where you can see it: the agent
...
A tool is registered against a package that is not installed. Tool registration
does not check that the handler exists, so it passes without a word and fails on
the first run.
...
That error comes from writing this course: the tools were registered, and the
...
**Debug Config** has five flags, and its help text lists four. `show_errors` is
the fifth.
</Aside>
