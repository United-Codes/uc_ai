...
<CardGrid>
  ...
  <LinkCard
    title="Agent as Tool"
    description="Register an agent as a normal tool that any caller can use"
    href="/products/uc-ai/docs/guides/multi-agent-systems/agent-as-tool/"
  />
  <LinkCard
    title="Handoff"
...
a handoff target, and an [agent as
tool](/products/uc-ai/docs/guides/multi-agent-systems/agent-as-tool/) all work on
the same document, and none of them can drop the binding.
...
An agent that has run cannot be deleted: its history points at it, and
`delete_agent` therefore stops with `ORA-02292`. That is the default on purpose,
because a deleted history cannot come back. You have three ways out.

**Keep everything, hide the agent.** This is the usual one. The agent stops
resolving, and every row stays:
...
**Delete a definition that never ran.** `delete_agent` removes one version and
nothing else.
...
<Aside type="caution">
  A purge cannot be undone. The rows are gone when you commit, and the token
  totals and the message log of those runs go with them. If you only want the
  agent out of the way, archive it.
</Aside>
...
A purge stops when another agent references this one, as a delegate of an
orchestrator, as a target of a handoff, or as a step of a workflow. Remove the
reference first, or the other agent points at nothing.
...
workflow step, an orchestrator sub-agent, a handoff hop, and an agent that runs
as a tool.
