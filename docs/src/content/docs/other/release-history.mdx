...
- **Durable execution telemetry**: execution rows are now written in autonomous transactions (like Logger). They are visible immediately to other sessions and survive a rollback of the calling transaction, so failed executions and their token accounting are never lost. **Breaking**: an agent must be committed before it can be executed in the same session transaction — creating and executing an agent in one uncommitted transaction now raises a clear error.
