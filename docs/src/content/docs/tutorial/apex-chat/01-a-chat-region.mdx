...
<LessonHeader
  ...
  of={6}
  minutes={20}
  promise="an APEX page has a working chat region, and you can see the one thing it still cannot do."
...
You need **UC AI 26.4 or later**, installed and able to reach a model. If a call
...
The version matters: the plug-in package calls `execute_agent` with
`p_run_context`, which is new in 26.4, so on an older version it does not
compile.
...
Agent](/products/uc-ai/docs/tutorial/service-contract-desk/01-the-scenario/). The
setup script below creates all of it, so you can start here.

If you already did that course, run the script anyway. It creates what is absent
and updates what is there, so it changes nothing.
</Aside>
...
It ends with a report. Every line must be good before you go on. To remove
...
A computation writes to session state. That is the difference, and it is hard to
see, because the item looks correct on the page.
</Aside>

## Send the first message

Run the page and ask the desk something it should know:
...
That is not a failure of the plug-in, and it is not the model being unhelpful.
The turn worked. The model called the right tool, the tool refused, and the model
...
The tool result is the interesting part:
...
<LinkCard
  ...
  href="/products/uc-ai/docs/tutorial/apex-chat/02-one-contract/"
/>
