...
Similar to [Structured Output](/products/uc-ai/docs/guides/structured_output) tools require you to describe the parameters in a [JSON Schema](#json-schema). If you have a tool that is called `GET_WEATHER` which has two parameters: `city` and `unit` (defaults to celsius), the schema could look like this:
