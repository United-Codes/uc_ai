...
Read the row with `get_prompt_profile`, change the columns that you need, and pass
...
The call selects the row by its `id`. It does not change `code`, `version`, or
`status`. To change the status, call [`change_status`](#changing-status).
