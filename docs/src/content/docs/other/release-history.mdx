...
- **Caller class** (`audience`): each run and each session also records the class of the caller — `public` for an anonymous APEX visitor, `authenticated` for a logged-in APEX user, and `db` for a database or job session. A username cannot express this distinction, because every anonymous visitor of a public page shares one identity.
- **Conditional step validation**: a step `condition` must be a string that holds a PL/SQL boolean expression. `create_agent` now rejects the object form with `ORA-20503` instead of silently ignoring it at run time.
...
- Mistral: `mistral-large`, `mistral-medium`, `mistral-small`, `magistral-medium`, `magistral-small`, `codestral`, `devstral-medium`, `devstral-small`, `ministral-8b`, `ministral-3b`, `pixtral-large`, plus the `mistral-embed` and `codestral-embed` embedding models. See [Mistral](/products/uc-ai/docs/providers/mistral/).
