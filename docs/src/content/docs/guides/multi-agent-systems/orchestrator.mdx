...
Choose the orchestrator when the right sequence of agents depends on the input and you want the AI to plan it. If the steps are always the same, a deterministic [workflow](/products/uc-ai/docs/guides/multi-agent-systems/workflows/) is simpler and cheaper.
</Aside>
...
The orchestrator makes the model calls itself and reaches its delegates through **tool calls**, so the [`uc_ai_agent_messages`](/products/uc-ai/docs/guides/multi-agent-systems/#conversation-message-log) transcript records each delegate as a `tool_call` / `tool_result` pair (`tool_name` = the delegate agent) around the orchestrator's own reasoning and final synthesis. Because this is a direct pattern, every agent-produced row is attributed to the orchestrator via `agent_code`; the delegate identity lives on the tool rows' `tool_name`.
