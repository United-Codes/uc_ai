---
title: Cheatsheet
description: One-page quick reference for the most-used UC AI functions, constants, and global settings.
sidebar:
  order: 5
---
...
A scannable reference for day-to-day work. For full parameter details see [generate_text](/products/uc-ai/docs/api/generate_text/) and [generate_embeddings](/products/uc-ai/docs/api/generate_embeddings/).
...
Model constants live in each provider package, e.g. `uc_ai_openai.c_model_gpt_4o_mini`, `uc_ai_anthropic.c_model_claude_3_5_haiku`, `uc_ai_google.c_model_gemini_2_5_flash`. See each [provider page](/products/uc-ai/docs/guides/providers/) for the full list, or use the [utility functions](/products/uc-ai/docs/api/utils/) to list them at runtime.
...
<Aside type="tip">
Use `p_config` when a library must not depend on or disturb the caller's globals. Use globals for straightforward application code where a single session owns the settings.
</Aside>
