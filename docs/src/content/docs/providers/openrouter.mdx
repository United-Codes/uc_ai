...
2. Oracle database with internet access to OpenRouter's API endpoints (see [Network Setup (ACL & Wallet)](/products/uc-ai/docs/guides/network-setup/))
...
Note: OpenRouter uses the OpenAI-compatible API format, so the `uc_ai_openai` package is used under the hood. Even so, set the credential on the `uc_ai_openrouter` package — it is resolved specifically for the OpenRouter provider.
