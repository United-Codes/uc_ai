...
- **`uc_ai_agent_exec_api.register_agent_as_tool` lost two parameters.** The signature is now `register_agent_as_tool(p_agent_code, p_tool_tag)`. The session and the parent execution come from the context of the orchestrator run. Code that calls this internal registration function must drop `p_exec_id` and `p_session_id`.
...
- **A tool handler gets one argument shape on every provider.** Some providers unwrap the declared object parameter and some do not. UC AI now unwraps it once, in the tools layer, for every provider. A handler that reads the wrapper object must read the parameters directly.
...
- **[Claude Code skills for consumers](https://github.com/United-Codes/uc_ai/tree/main/skills)**: the repository now ships agent skills that teach Claude Code (and other agents that read the skill format) how to call UC AI — quickstart, tools, reasoning, structured output, prompt profiles, multi-agent, agent memory, file analysis, and event callbacks. Copy the ones you need into your project's `.claude/skills/` directory.
