...
After you log in to the [Oracle Cloud Console](https://cloud.oracle.com/), you can create an API key for the OCI provider in UC AI.
...
Give the Web Credential a `Name` and a `Static ID`. We recommend that both match.
...
Open your downloaded private key file in a text editor (`*.pem` file). Only copy the secret **without** the `-----BEGIN PRIVATE KEY-----` and `-----END PRIVATE KEY-----` lines.
