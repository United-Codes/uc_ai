...
The plug-in is not part of the UC AI installation. Download the
`uc-ai-chat-*.zip` file from the [latest UC AI
release](https://github.com/United-Codes/uc_ai/releases/latest). The archive
holds the table script, the two packages, the optional hook package, and the
plug-in export file.
</Aside>
...
a working region on an agent in six lessons, and it covers the parts this page
lists as attributes.
...
- **UC AI 26.3 or later** in the database schema. The package of the plug-in
...
1. ### Create the table
   ...
   Run `ai_tables.sql`. This script creates `uc_ai_chat_messages`, which holds
...
2. ### Install the packages
   ...
   Run `uc_ai_chat.pks` and then `uc_ai_chat.pkb`. The package body calls
   `uc_ai_agents_api`, so UC AI must be installed first.

3. ### Import the plug-in

   Import `region_type_plugin_com_united_codes_uc_ai_chat.sql` into your APEX
   application. The file is a normal APEX component export.

4. ### Optional: show the tool progress

   Run `uc_ai_chat_hook.pks` and then `uc_ai_chat_hook.pkb`. With this package,
   the chat shows the name of the tool that runs now.
...
The current plug-in needs UC AI 26.3, because the package body calls
...
| Auto Generate Title, Collect Feedback, Agent Run Context, Run Context Drift | 26.3 |
...
<LinkCard
  ...
  description="Six lessons that put this region on a page: one contract, private conversations, and an audit trail that names the person who asked."
