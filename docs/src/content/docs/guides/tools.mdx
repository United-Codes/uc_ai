...
<Aside type="tip" title="Want the shortest example first?">
The [Quickstart](/products/uc-ai/docs/guides/quickstart/) registers a minimal, parameter-free tool and shows the model calling it. This guide goes deeper: parameters, best practices, and real read/write examples.
</Aside>
...
`create_tool_from_schema` is **create-only** — if a tool with the same `p_tool_code` already exists it will raise an error.

### Upserting a tool with `merge_tool_from_schema`

If you want to create a tool if it doesn't exist yet, or update its description, parameters, and tags if it does, use `uc_ai_tools_api.merge_tool_from_schema` instead. It takes the exact same parameters as `create_tool_from_schema` and is useful for idempotent deployment scripts:
...
On update, the tool's parameters and tags are fully replaced — existing rows in `uc_ai_tool_parameters` and `uc_ai_tool_tags` for that tool are removed and rewritten from the new schema/tags.
...
The tools above run **inside your database**. Most providers also offer their own
**server-side tools** that run on their infrastructure — Anthropic web search,
the OpenAI Responses API web search, Google search, and more. UC AI can pass
these through: set `uc_ai.g_provider_tools` to a JSON array of raw,
provider-native tool definitions and they are appended verbatim to the request's
`tools` array (executed by the provider, never locally):
...
These combine with your local tools and are sent even when `g_enable_tools` is
`false`. See
