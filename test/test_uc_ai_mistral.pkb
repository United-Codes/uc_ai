create or replace package body test_uc_ai_mistral as
  -- @dblinter ignore(g-5010): allow logger in test packages
  -- @dblinter ignore(g-2160): allow initialzing variables in declare in test packages

  c_test_model constant uc_ai.model_type := uc_ai_mistral.c_model_mistral_small;
  -- Mistral has no reasoning parameter; reasoning comes from picking a magistral model
  c_reasoning_model constant uc_ai.model_type := uc_ai_mistral.c_model_magistral_small;
  c_embeddings_model constant uc_ai.model_type := uc_ai_mistral.c_model_mistral_embed;
  ...
  begin
    ...
    -- Set provider override for Mistral
    uc_ai.g_base_url := 'https://api.mistral.ai/v1';
    uc_ai.g_provider_override := uc_ai.c_provider_mistral;
  ...
  as
    ...
    l_final_message clob;
  begin
    -- Mistral has no reasoning parameter; the magistral models reason by default.
    -- enable_reasoning must not break the request (no unknown params are sent).
