...
Choose a conversation when several perspectives should build on each other — debate, review, or brainstorming — rather than running a fixed pipeline. For a single decision-maker delegating work, use the [orchestrator](/products/uc-ai/docs/guides/multi-agent-systems/orchestrator/).
</Aside>
...
A conversation turn is persisted to [`uc_ai_agent_messages`](/products/uc-ai/docs/guides/multi-agent-systems/#conversation-message-log) as a **full transcript**: the opening input as a `user` row, then one `assistant` row per participant turn, each attributed to the agent that produced it via `agent_code`. In AI-driven mode the moderator's closing summary is the final row. There are no tool rows — a conversation is discussion, not tool use.
...
This makes the whole debate queryable — filter by `agent_code` to isolate one participant's contributions, or read in `seq` order to replay the discussion.
