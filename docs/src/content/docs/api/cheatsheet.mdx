...
<Aside type="tip">
Use `p_config` when a library must not depend on or disturb the caller's globals. Use globals for straightforward application code where a single session owns the configuration.
</Aside>
