...
Instead of manually setting the provider setting UC AI also lets you simply choose between 3 levels: `low`, `medium`, `high`. In that case you don't need to supply any provider specific settings. Although we recommend for real productive systems to experiment and fine-tune reasoning settings for you chosen provider and model.

You can use these generic settings via the `g_reasoning_level` variable in the `uc_ai` package.
...
OpenAI's Responses API only returns reasoning content when you opt in — see [Reasoning content, storage, and encryption](/products/uc-ai/docs/providers/openai/#reasoning-content-storage-and-encryption) for how `g_store_responses`, `g_include_encrypted_reasoning`, and `g_reasoning_summary` affect what you get back.
