...
- `g_enable_programmatic_tools`: Also offer [code mode](/products/uc-ai/docs/guides/programmatic-tool-calling) — the `uc_ai__run_code` meta-tool — alongside the normal tools (boolean; requires `g_enable_tools` and the MLE sandbox)
...
- `g_extra_headers`: Additional HTTP request headers sent with every provider request (object of name/value pairs, e.g. `{"X-Tenant-Id": "acme"}`)
