...
The UC AI Ollama package integrates the open-source language models of a locally running Ollama instance into your Oracle database applications.
...
- Multi-modal support (text and images) with models that support it. **Ollama cannot read a PDF**: it answers `HTTP 400: file inputs are not currently supported`. Convert each page to a PNG file and send one image for each page.
- No API keys required (runs locally)

## Responses API

As of v26.2, UC AI routes Ollama calls through Ollama's OpenAI-compatible Responses API endpoint by default. To fall back to the legacy chat endpoint, set:
...
3. Make sure that the Oracle database can access the Ollama API endpoint — you still need a network ACL for the Ollama host (e.g. `localhost`), see [Network Setup (ACL & Wallet)](/products/uc-ai/docs/guides/network-setup/)
...
By default, the package connects to `http://localhost:11434/api/chat`. If your Ollama instance runs on a different host or port, set the `uc_ai.g_base_url` global variable before you call `uc_ai.generate_text`.
...
Refer to the [list of models that support tools](https://ollama.com/search?c=tools) to find models that can use function calling. Small models can support tools, but they are less capable in complex tasks. If you enable reasoning, the model can call the tools more effectively.
